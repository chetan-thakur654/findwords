...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${lNum} Letter Words With  ${letterString}`,
    description: `Find all ${lNum} letter words containing ${letterString} for Wordle, Words with Friends, unscrambler, Crosswords, Puzzles and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/containing-letters/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function ContainingLetters({ params }) {
  const { word_finder, letter } = params;
  ...
  const letterArray = letter.split("-");
  console.log(letter, letterArray);
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>{`${lNum} Letter Words Containing  ${letterString} `} </h1>
          <p>
            {`Find all ${lNum} letter words with ${letterString} for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words that contains ${letterString} in seconds.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words with ${letterString} in our database at the
              moment`}
            </h2>
          )}
        </section>
