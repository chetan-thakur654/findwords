...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${lNum} Letter Words With ${letter.toUpperCase()} As Fourth letter`,
    description: `Find all ${lNum} letter words with ${letter.toLowerCase} as fourth letter for Wordle, Words with Friends, unscrambler, Crosswords, Puzzles and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/fourth-letter/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function FourthLetter({ params }) {
  const { word_finder, letter } = params;
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>
            {`${lNum} Letter Words With ${letter.toUpperCase()} As Fourth Letter `}
          </h1>
          <p>
            {`Find all ${lNum} letter words with ${letter.toUpperCase()} as fourth letter for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words with  ${letter.toUpperCase()} as fourth letter in fourths.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words with ${letter.toUpperCase()} as fourth letter in our database at the
              moment`}
            </h2>
          )}
        </section>
