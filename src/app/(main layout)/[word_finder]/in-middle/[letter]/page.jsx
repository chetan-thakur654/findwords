...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `Find all ${lNum} letter words with ${letter.toLowerCase} in the middle for Wordle, Words with Friends, unscrambler, Crosswords, Puzzles and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/in-middle/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function InMiddle({ params }) {
  const { word_finder, letter } = params;
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>
            {`${lNum} Letter Words With ${letter.toUpperCase()} in The Middle `}
          </h1>
          <p>
            {`Find all ${lNum} letter words with ${letter.toUpperCase()} in the middle for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words with  ${letter.toUpperCase()} in the middle in seconds.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words with ${letter.toUpperCase()} in the middle in our database at the
              moment`}
            </h2>
          )}
        </section>
