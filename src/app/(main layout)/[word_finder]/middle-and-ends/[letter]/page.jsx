...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${lNum} Letter Words  With ${letterArray[0].toUpperCase()} In The Middle And ${letterArray[1].toUpperCase()} at The End`,
    description: `Find all ${lNum} letter words with  ${letterArray[0].toUpperCase()} in the middle and ${letterArray[1].toUpperCase()} at the end for Wordle, Words with Friends, unscrambler and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/middle-and-ends/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function MiddleAndEnd({ params }) {
  const { word_finder, letter } = params;
  ...
  const data = NoOfletters.words.filter((word) => {
    ...
    return (
      ...
      lowerCaseWord.endsWith(letterArray[0])
    );
  });
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>{`${lNum} Letter Words With ${letterArray[0].toUpperCase()} in the Middle And ${letterArray[1].toUpperCase()} at the End `}</h1>
          <p>
            {`Find all ${lNum} letter words with ${letterArray[0].toUpperCase()} in the middle and  ${letterArray[1].toUpperCase()} at the end for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words with ${letterArray[0].toUpperCase()} in the middle and  ${letterArray[1].toUpperCase()} at the end in seconds.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words that with ${letterArray[0].toUpperCase()} in the middle and  ${letterArray[1].toUpperCase()} at the end,  in our database at the
              moment`}
            </h2>
          )}
        </section>
