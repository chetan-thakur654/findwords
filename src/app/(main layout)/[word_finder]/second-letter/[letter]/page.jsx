...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${lNum} Letter Words With ${letter.toUpperCase()} As Second letter`,
    description: `Find all ${lNum} letter words with ${letter.toLowerCase} as second letter for Wordle, Words with Friends, unscrambler, Crosswords, Puzzles and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/second-letter/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function SecondLetter({ params }) {
  const { word_finder, letter } = params;
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>
            {`${lNum} Letter Words With ${letter.toUpperCase()} As Second Letter `}
          </h1>
          <p>
            {`Find all ${lNum} letter words with ${letter.toUpperCase()} as second letter for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words with  ${letter.toUpperCase()} as second letter in seconds.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words with ${letter.toUpperCase()} as second letter in our database at the
              moment`}
            </h2>
          )}
        </section>
        ...
        <div className="sidebar-Item">
          <div className={styles["other-words-finder"]}>
            ...
            <div className="finder-list">
              <ul>
                {mainPages &&
                  mainPages.map(({ name, link, data }, index) => (
                    <li key={index}>
                      <Link href={`/${link}`}>
                        <div>
                          <h3>{name}</h3>
