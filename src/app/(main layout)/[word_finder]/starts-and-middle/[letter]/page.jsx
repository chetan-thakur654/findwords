...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `Find all ${lNum} letter words that starts with  ${letterArray[0].toUpperCase()} and ${letterArray[1].toUpperCase()} in the middle for Wordle, Words with Friends, unscrambler and more`,
    alternates: {
      canonical: `https://findwords.co/${word_finder}/starts-and-middle/${letter}`,
      // languages: {
      //   'en-US': '/en-US',
      //   'de-DE': '/de-DE',
      // },
...
export default function StartAndMiddle({ params }) {
  const { word_finder, letter } = params;
  ...
  return (
    <div className={styles.home}>
      <div className={styles.left}>
        <section>
          <h1>{`${lNum} Letter Words That Starts With ${letterArray[0].toUpperCase()} And ${letterArray[1].toUpperCase()} in the Middle `}</h1>
          <p>
            {`Find all ${lNum} letter words starting with ${letterArray[0].toUpperCase()} and  ${letterArray[1].toUpperCase()} in the middle for Wordle, Words with Friends, scrambler, anagrams and more`}
          </p>
          <p>
            {`We have created the ${lNum} Letter Words Finder, a simple tool that helps
            you find any ${lNum} letter words that starts with ${letterArray[0].toUpperCase()} and  ${letterArray[1].toUpperCase()} in the middle in seconds.`}
          </p>
          {data.length > 0 ? (
            <WordList data={data} />
          ) : (
            <h2>
              {` Sorry ! We don't have ${lNum} letter words that starts with ${letterArray[0].toUpperCase()} and  ${letterArray[1].toUpperCase()}in the middle,  in our database at the
              moment`}
            </h2>
          )}
        </section>
